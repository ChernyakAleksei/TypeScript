...
namespace ts.DocumentHighlights {
    export function getDocumentHighlights(program: Program, typeChecker: TypeChecker, cancellationToken: CancellationToken, sourceFile: SourceFile, position: number, sourceFilesToSearch: SourceFile[]): DocumentHighlights[] {
        const node = getTouchingWord(sourceFile, position);
        return node && (getSemanticDocumentHighlights(node, program, typeChecker, cancellationToken, sourceFilesToSearch) || getSyntacticDocumentHighlights(node, sourceFile));
    }
    ...
    function getSemanticDocumentHighlights(node: Node, program: Program, typeChecker: TypeChecker, cancellationToken: CancellationToken, sourceFilesToSearch: SourceFile[]): DocumentHighlights[] {
        const referenceEntries = FindAllReferences.getReferenceEntriesForNode(node, program, sourceFilesToSearch, typeChecker, cancellationToken);
